...
function statusLabelForSource(source: ManualValueSourceMetadata | undefined, value: number | undefined): string {
  if (value === undefined || !Number.isFinite(value)) return 'وارد نشده';
  if (!source) return 'مقدار ذخیره‌شده قدیمی / stale';
  if (source.source === 'manual') return value === 0 ? 'صفر تأییدشده توسط کاربر' : 'وارد شده دستی';
  if (source.source === 'user-confirmed-zero') return 'صفر تأییدشده توسط کاربر';
  if (source.stale) return 'مقدار ذخیره‌شده قدیمی / stale';
  if (source.source === 'tsetmc-suggestion') return 'اعمال‌شده از پیشنهاد TSETMC';
  if (source.source === 'codal-excel-manual-review') return 'اعمال‌شده از بررسی دستی Excel کدال';
  if (source.source === 'financial-statement-suggestion') return 'اعمال‌شده از صورت مالی کدال';
  if (source.source === 'codal-suggestion') return 'اعمال‌شده از پیشنهاد کدال';
...
export function buildNavCompletionSummary(
  ...
  const fields = workflowFields.map((field): NavCompletionFieldState => {
    ...
    return {
      ...
      value,
      statusLabel: statusLabelForSource(source, value),
